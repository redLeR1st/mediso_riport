...
Az általam megvizsgált írások között felelhetõek atlasz alapú, gépi tanulás, illetve egyszerû algoritmikus alapokon nyugvó módszerek.
Az alábbi sorokban ezek közül összegzek.

\section{Borda címkézés}
\subsection{Bordák szegmentálása alacsony sugárzású CT képek segítségével \cite{SegmOfRibs}}

\textbf{Tesztelt képek:}

115 alacsony sugárzással felvett mellkasi CT képek, különbözõ szelet vastagsággal.
5 CT felvétel az algoritmus paramétereinek a beállítására, 110 CT felvételt az algoritmus
kiértékelésére használtak
98%-os pontosság
...
Gerinccsatorna megrajzolása

A kezdõpont (seed point) automatikusan kerül meghatározásra a legalsó projekcióból. A
csatorna iteratívan kerül megrajzolásra az aktuális(Ii) és a rákövetkezõ(Ii-1) képszeletbõl.
...
3 Találjuk meg azt a pontot a rákövetkezõ képen ami a legközelebb van az aktuális képen
kiszámolt ponthoz (1 lépésben ez a kezdõpont)

4 Ismételjük az elsõ és a második lépést

5 a megtaláltpontok együtt alkotják a gerinccsatorna középpontját
...
Az ebben a cikkben tárgyalt módszerek 15 kutatásból lettek kiválogatva, melyek 1999 és 2013
...
\textbf{1} 2010, Furuhashi et al. módszere:
...
\textbf{2} 2010, Banik et al.módszere:
...
axiális szeletek meghatározzák a középvonalat amely végig megy a gerincen (? talán a
gerincsatornára gondolnak). Ez azért kell, mert ha egy régió megfelelõ távolságra van ettõl a
ponttól, (és átment a küszöbölésen) akkor az a borda része. Ezután a megtalált borda
szerkezetének a fejlesztése további morphológiai mûveleteket alkalmaznak.

\textbf{3} 2009, the work of Zhou et al. módszere:
...
\textbf{1} 2004, Shen et al. módszere:

Magpont keresése a bordákon, a mellkashoz közel, hasonlóan
mint a hogy azt az elõzõ cikkben is tárgyaltuk, majd ebbõl a magpontból bordák növesztése.

\textbf{2} 2010, H. Li et al. módszere:
...
\textbf{3} 2012, Zhang et al. módszere:

Korona szeletek követésével képes teljesen elválasztott 12
bordát szegmentálni, a középsõ szeletbõl kiindulva. A lehetséges bordát tartalmazó
területeket 110HU-nál thresholdolták

\textbf{4} 2012, Yao et al. módszere: 

A gerinc szegmentálásával kezdõdik. A bordák 200HU felett és
2cm-re a szegmentált gerinctõl detektálódnak. Majd egy kereszt sekciót (cross section)
definiálunk a detektált szerkezet közepére. Az elsõ cross section meghatározása után az
algoritmus egy B-spline görbét. Ezzel a görbével határozzuk meg a következõ cross sectiont.

\textit{-Régió növelés (Region growing)}

\textbf{1} 2002, Kim et al. módszere:

Hibrid módszer rekurzív megrajzolással (recursive tracing) és
régiónöveléssel. A célobjektum a felhasználó által megadott iniciális pontból növeszti ki.
...
\textbf{2} 2012, Ramakrishnan et al. módszere 3D középvonal kinyerése a torakális CT-bõl. A Módszer
két részre osztható.
...
b) A magpont meghatározása után következik a tracing Radnom Walker algoritmust
használva.


\textbf{Priorinformáció alakú módszerek}

\textit{-Borda váz modell alapján}

\textbf{1} 2012 Wu et al. módszere:

A borda magpontját mintaillesztéssel, tanulás alapú objektum
középvonal detektáló algoritmussal. Tanítóadatbázisnak 40CT felvételt használtak, melyet
...
\textbf{1} 2013, Gargouri et al. módszere:
...
Invariant Shape Context Descriptor. Mindegyik csont voxelre kiszámoltak 56 sávot (képen 2
sáv van kék és piros) melyeknek a sugarai random választották 2 és 35 között. Aztán az összes
sávra kiszámolták, hogy hány csont voxelt érint. Ebbõl pedig elõáll a jellemzõ.

\textbf{2} 2007, Staal et al. 5 lépéses módszere:
...
tartjuk meg melyek a legjobb a legjobb teljesítményt nyújtják.
...
Intervention (MICCAI) Workshop-on. A módszereket 10 tanítóképen, és 5 testképen
