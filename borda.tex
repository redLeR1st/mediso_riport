...
\centering
\caption{Borda címkézõ algoritmus lépései} \label{fig:alg1}
\end{figure}
\section{A csont szegmentálása}
...
\subsection{Az Otsu szegmentálás}
Otsu szegmentálási módszere, olyan automatikus megoldást nyújt, mely segítségével elég csak a kép hisztogramját felhasználnunk a küszöbérték meghatározására.
Az algoritmus lényegében azt a \textit{t} küszöbértéket keresi, amely minimalizálja az elõtér és a háttér közötti varianciát. \cite{otsu}
\begin{figure}[htp]
...
\end{figure}
A módszer \aref{eq:var}-es egyenlet minimalizálásával találja meg az optimális \textit{t} küszöbértéket, ahol $\omega_0(t)$ és $\omega_1(t)$ a \textit{t} küszöbérték által elválasztott osztályok valószínûsége (elõtér, háttér), $\sigma_0^2$ és $\sigma_1^2$ pedig az osztályok varianciája.
Szemléletesen \aref{fig:otsu_hist} ábrán láthatjuk a módszer eredményét ahol a piros függõleges vonal az optimális küszöbértéket jelöli.
...
\subsection{A csont szegmentálása CT képeken}
...
Elég pusztán a CT felvétel hisztogramját használnunk a megfelelõ threshold érték megkeresésére.
...
Az adott kép hisztogramját 0-tót lefelé illetve 1000-tõl felfelé eldobjuk.
...
\section{A gerinc kijelölése}
A gerinc helyzetének meghatározása kulcsfontosságú a bordadetektálás szempontjából.
Tudjuk, hogy a bordák, a gerinchez kapcsolódnak, ezért, ha ismerjük a gerinc helyzetét, könnyen megtalálhatjuk a csigolyákhoz tartozó bordákat is, a gerinc mentét pásztázva. Az algoritmus lépései \aref{fig:gerinc_ki} ábrán láthatóak.

\subsection{A Hough transzformáció}
...
A kör egyenlete kettõ dimenziós térben \aref{eq:kor}-es egyenlettel írható le.
\begin{equation} \label{eq:kor}
(x-a)^2 + (y-b)^2 = r^2
\end{equation}
Az egyenletben \textit{a} és \textit{b} a kör középpontja, \textit{r} pedig a sugár.
Ha a két dimenziós pont \textit{(x,y)} rögzítve van akkor a paraméterek \aref{eq:kor}-es egyenlet alapján megtalálhatóak.
...
Ezt a mátrixot nevezzük akkumulátor képnek.
A keresett kör \aref{fig:hough} ábra baloldali koordináta rendszerében helyezkedik el.
E körön válasszunk ki például három pontot, de az objektum többi pontja is felhasználható, majd e három pont körül rajzoljunk köröket az akkumulátor képen, úgy, hogy minden körön lévõ pixelt eggyel növeljünk a mátrixban.
...
Különbözõ sugarú köröket is detektálhatunk e módszer segítségével, ekkor az \textit{r} paraméter új dimenzióként bekerül az akkumulátorképbe.

\begin{figure}[!tbp]
\centering
	\includegraphics[width=0.5\textwidth]{hough_kor}
	\caption{Kép és akkumulátor kép \cite{akum}} \label{fig:hough}
\end{figure}


\subsection{A körök keresése axiális szeleteken}
...
Az algoritmus eredménye \aref{fig:korrig_0_58} ábrán látható.
%Az algoritmus eredménye \aref{fig:korrig_0_60} ábrán látható.
\subsection{A körök helyzetének korrigálása}
Elõfordulhat azonban, hogy a keresést zaj, kevés csont vagy más kör alakú objektumok (például aorta) jelenléte nehezíti meg.
...
Az algoritmus eredménye \aref{fig:korrig_1_58} ábrán látható.
...
\subsection{A körök helyzetének második korrigálása}
...
A rossz helyen lévõ kört ezzel a kiszámolt értékkel helyettesítjük.
A módszer az átlagoló/medián szûrõhöz hasonló elven mûködik.
Az algoritmus eredménye \aref{fig:korrig_2_58} ábrán látható.
...
\begin{figure}[!tbp]
	...
	\begin{minipage}[b]{0.3\textwidth}
		...
		\caption{Korrigálatlan} \label{fig:korrig_0_58}
	\end{minipage}
	\hfill
	\begin{minipage}[b]{0.3\textwidth}
		\includegraphics[width=\textwidth]{korrig_1_58}
		\caption{Elsõ korrigálás} \label{fig:korrig_1_58}
	\end{minipage}
	\hfill
	\begin{minipage}[b]{0.3\textwidth}
		\includegraphics[width=\textwidth]{korrig_2_58}
		\caption{Második korrig} \label{fig:korrig_2_58}
	\end{minipage}
\end{figure}
...
\section{A magpontok meghatározása}
...
A régiónövelés után ideális esetben elõáll a kép ami csak a bordákat tartalmazza.
...
A különbözõ bordáknál is megfigyelhetõ hogy két nagy illetve egy kicsi sajátértékkel rendelkeznek, ezt kihasználva elhagyhatjuk az ennek a tulajdonságnak nem megfelelõ objektumokat.
...
\begin{figure}[h]
	...
	\caption{Különbözõ objektumok sajátértékei \cite{hese}} \label{fig:sajat_ert}
\end{figure}

\section{A bordák címkézése újranövesztéssel}
...
Az címkézõ algoritmus bemenete csak a bordákat tartalmazza, mivel az elõzõ lépésben eltávolítottuk az egyéb objektumokat, de fontos, hogy a bordáknak külön címkéjük legyen, vagyis külön objektumként jelenjenek meg, ezt újabb régiónövelés tudja biztosítani.
A keresés ugyan olyan módszerrel történik, mint az 5.3-as fejezetben leírt magpont meghatározás, azzal a különbséggel, hogy itt már biztosak lehetünk benne, hogy csak bordák fordulhatnak elõ.
...
\section{A bordák címkézése bounding boxszal}
...
Az elõzõ alfejezetben is említésre került, hogy a régiónövelés definiál egy anatómiailag helytelen címkézését.
...
A bounding boxszal való címkézés jobb eredményeket produkál mint az újranövesztéses módszer.
Az újranövesztéssel 70\%-os pontosság érhetõ el, míg a bounding boxszal 79\%.
